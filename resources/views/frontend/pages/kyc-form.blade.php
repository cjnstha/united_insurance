					...
					<div class="tab_nav_side">
					  ...
					  <ul class="nav-side nav-tabs  tabs-left"><!-- 'tabs-right' for right tabs -->
					    ...
					    <li><a href="{{url('/agm-minute')}}"> AGM Minute <i
                            ...
                            <li class="active"><a href="{{url('/kyc-form')}}">KYC Forms <i class="fa fa-angle-right"></i></a></li>
				...
				<div class="download__container col-lg-8 col-md-8 col-sm-6 col-xs-12 pull-right key_benefits">
                    <div class="download__container__table" data-url="agent-form-details">
                            <table class="table table-sm">
                                ...
                                @forelse($kycs as $kyc)
                                    <tr>
                                        <td data-label="Name" title="Personal Form">{{$kyc->kyc_name}}</td>
                                        <td data-label="Date Added">{{$kyc->updated_at->toFormattedDateString()}}</td>
                                        <td data-label="View File">
                                            <a class='btn btnViewFile' data-target="{{asset("storage/$kyc->kyc_file")}}" data-title="{{$kyc->kyc_name}}" data-modal="downloadModal" title="View File"><i class='fa fa-eye'></i> View</a>
                                        </td>
                                        <td data-label="Download File">
                                            <a href="../public/storage/{{$kyc->kyc_file}}" title="Download Attachment" class='btn btnViewFile' download target='_blank'><span><i class="fa fa-file-pdf-o" aria-hidden="true"></i></span> Download <i class="fa fa-download" aria-hidden="true"></i></a>
                                        </td>
                                    </tr>
                                @empty
                                @endforelse
